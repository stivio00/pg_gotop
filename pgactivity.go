...
type PgStatActivity struct {
	...
	BackendStart    string //Time when this process was started. For client backends, this is the time the client connected to the server.
	XactStart       string //Time when this process' current transaction was started, or null if no transaction is active. If the current query is the first of its transaction, this column is equal to the query_start column.
	QueryStart      string //Time when the currently active query was started, or if state is not active, when the last query was started
	StateChange     string //Time when the state was last changed
